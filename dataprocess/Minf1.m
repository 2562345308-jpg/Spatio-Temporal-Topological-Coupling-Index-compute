...
clc;
% 输入和输出的基本路径
inputBasePath = 'G:\Matrixparameter\R06';%%%%%
outputBasePath = 'G:\Meanparameter\R06';%%%%%
...
for i = 1:length(subdirs)
    ...
    if exist(resultsPath, 'dir')
        ...
        matFiles = dir(fullfile(resultsPath, '*P-C.mat'));
        ...
        if ~isempty(matFiles)
            ...
            for j = 1:length(matFiles)
                ...
                efficiencyValues = [efficiencyValues, data.sum_C]; % 假设.mat文件中的数据在字段"assortativity"中
            end
            
            % 计算平均值
            averageinf = mean(efficiencyValues);
            ...
            % 保存平均值为.mat文件
            outputFilePath = fullfile(outputSubdirPath, sprintf('average_P-C.mat'));
            save(outputFilePath, 'averageinf');
        end
    end
end
