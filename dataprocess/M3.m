...
for i = 1:length(subdirs)
    ...
    if exist(resultsPath, 'dir')
        ...
        matFiles = dir(fullfile(resultsPath, '*transitivity.mat'));
        ...
        if ~isempty(matFiles)
            ...
            for j = 1:length(matFiles)
                ...
                transitivityValues = [transitivityValues, data.transitivity]; % 假设.mat文件中的数据在字段"assortativity"中
            end
            
            % 计算平均值
            averagetransitivity = mean(transitivityValues);
            ...
            % 保存平均值为.mat文件
            outputFilePath = fullfile(outputSubdirPath, sprintf('average_transitivity.mat'));
            save(outputFilePath, 'averagetransitivity');
        end
    end
end
