...
% 遍历每个子文件夹
    for i = 1:length(subfolders)
        ...
        % 构建.mat文件的完整路径
        matFilePath = fullfile(currentFolderPath, 'average_P-P.mat');%%%%%%%%%
    
        % 加载.mat文件数据
        loadedData = load(matFilePath);
        combinedData = [combinedData, loadedData.averageinf];
    end
    % 构建保存目标文件的完整路径
    ...
    targetFilePath = fullfile(targetFolder1, sprintf('signal_PP.mat'));%%%%%%%%%%%
