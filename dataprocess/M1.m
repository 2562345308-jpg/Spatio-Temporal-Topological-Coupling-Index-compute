...
for i = 1:length(subdirs)
    ...
    if exist(resultsPath, 'dir')
        ...
        matFiles = dir(fullfile(resultsPath, '*assortativity.mat'));
        ...
        if ~isempty(matFiles)
            ...
            for j = 1:length(matFiles)
                ...
                assortativityValues = [assortativityValues, data.assortativity]; % 假设.mat文件中的数据在字段"assortativity"中
            end
            
            % 计算平均值
            averageAssortativity = mean(assortativityValues);
            ...
            % 保存平均值为.mat文件
            outputFilePath = fullfile(outputSubdirPath, sprintf('average_assortativity.mat'));
            save(outputFilePath, 'averageAssortativity');
        end
    end
end
