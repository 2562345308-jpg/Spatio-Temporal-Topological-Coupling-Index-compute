...
for i = 1:length(subDirs)
    if subDirs(i).isdir && ~strcmp(subDirs(i).name, '.') && ~strcmp(subDirs(i).name, '..')
        ...
        if isfolder(subSubDirsN1)
            matFiles = dir(fullfile(subSubDirsN1, 'average_clustering_2*.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN1, matFiles(j).name));
                x1 = [x1 matFile.averageclustering2];%%%
            end
        end
        
        % 处理Sleep stage N2_
        if isfolder(subSubDirsN2)
            matFiles = dir(fullfile(subSubDirsN2, 'average_clustering_2*.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN2, matFiles(j).name));
                x2 = [x2 matFile.averageclustering2];%%%
            end
        end
        
        % 处理Sleep stage N3_
        if isfolder(subSubDirsN3)
            matFiles = dir(fullfile(subSubDirsN3, 'average_clustering_2*.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN3, matFiles(j).name));
                x3 = [x3 matFile.averageclustering2];%%%
            end
        end
        if isfolder(subSubDirsN4)
            matFiles = dir(fullfile(subSubDirsN4, 'average_clustering_2*.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN4, matFiles(j).name));
                x4 = [x4 matFile.averageclustering2];%%%
            end
        end
        if isfolder(subSubDirsN5)
            matFiles = dir(fullfile(subSubDirsN5, 'average_clustering_2*.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN5, matFiles(j).name));
                x5 = [x5 matFile.averageclustering2];%%%
            end
        end
    end
end
