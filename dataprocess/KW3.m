...
filePattern = 'average_transitivity.mat';%%%%
...
for groupIdx = 1:length(subDirs)
    ...
    for subSubDirIdx = 1:length(subSubDirs)
        ...
        if ~isempty(matFiles)
            ...
            transitivityData = load(matFilePath);
            transitivityValue = transitivityData.averagetransitivity;%%%%%%%
            
            % Add assortativity value to the respective group's array
            switch groupIdx
                case 1
                    x0 = [x0, transitivityValue];
               
                case 2
                    x2 = [x2, transitivityValue];
                case 3
                    x3 = [x3, transitivityValue];
                
                case 4
                    x8 = [x8, transitivityValue];
                case 5
                    x9 = [x9, transitivityValue];
            end
        end
    end
end
...
[p_kw, tbl, stats_kw] = kruskalwallis(b,group); % 将矩阵作为参数传递给函数
