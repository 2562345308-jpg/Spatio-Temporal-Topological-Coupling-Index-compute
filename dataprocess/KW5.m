...
filePattern = 'average_clustering_8.mat';%%%%
...
for groupIdx = 1:length(subDirs)
    ...
    for subSubDirIdx = 1:length(subSubDirs)
        ...
        if ~isempty(matFiles)
           ...
           clusteringData = load(matFilePath);
           clusteringValue = clusteringData.averageclustering8;%%%%%%%
            
            % Add assortativity value to the respective group's array
            switch groupIdx
                case 1
                    x0 = [x0,  clusteringValue];
               
                case 2
                    x2 = [x2,  clusteringValue];
                case 3
                    x3 = [x3,  clusteringValue];
               
                case 4
                    x8 = [x8,  clusteringValue];
                case 5
                    x9 = [x9,  clusteringValue];
            end
        end
    end
end
