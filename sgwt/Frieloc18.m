...
clc;
% 设置目录路径
baseDir = 'E:\processed_data\B1\G9';%%%
...
for i = 1:length(subDirs)
    if subDirs(i).isdir && ~strcmp(subDirs(i).name, '.') && ~strcmp(subDirs(i).name, '..')
        subSubDirsN1 = fullfile(baseDir, subDirs(i).name);
        subSubDirsN2 = fullfile(baseDir, subDirs(i).name);
        subSubDirsN3 = fullfile(baseDir, subDirs(i).name);
        subSubDirsN4 = fullfile(baseDir, subDirs(i).name);
        subSubDirsN5 = fullfile(baseDir, subDirs(i).name);
        
        % 处理Sleep stage N1_
        if isfolder(subSubDirsN1)
            matFiles = dir(fullfile(subSubDirsN1, 'ratio1_Eloc8.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN1, matFiles(j).name));
                x1 = [x1 matFile.ratio1];%%%
            end
        end
        
        % 处理Sleep stage N2_
        if isfolder(subSubDirsN2)
            matFiles = dir(fullfile(subSubDirsN2, 'ratio2_Eloc8.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN2, matFiles(j).name));
                x2 = [x2 matFile.ratio2];%%%
            end
        end
        
        % 处理Sleep stage N3_
        if isfolder(subSubDirsN3)
            matFiles = dir(fullfile(subSubDirsN3, 'ratio3_Eloc8.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN3, matFiles(j).name));
                x3 = [x3 matFile.ratio3];%%%
            end
        end
        if isfolder(subSubDirsN4)
            matFiles = dir(fullfile(subSubDirsN4, 'ratio4_Eloc8.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN4, matFiles(j).name));
                x4 = [x4 matFile.ratio4];%%%
            end
        end
        if isfolder(subSubDirsN5)
            matFiles = dir(fullfile(subSubDirsN5, 'ratio5_Eloc8.mat'));%%%
            for j = 1:length(matFiles)
                matFile = load(fullfile(subSubDirsN5, matFiles(j).name));
                x5 = [x5 matFile.ratio5];%%%
            end
        end
    end
end
